...
import com.example.latihansqlite_mynotesapp.adapter.NoteAdapter
import com.example.latihansqlite_mynotesapp.databinding.ActivityMainBinding
import com.example.latihansqlite_mynotesapp.db.NoteHelper
import com.example.latihansqlite_mynotesapp.entity.Note
import com.example.latihansqlite_mynotesapp.helper.MappingHelper
...
class MainActivity : AppCompatActivity() {
    ...
    private fun deleteNote(note: Note, position: Int) {
        ...
        if (result > 0) {
            ...
            showSnackbarMessage("Failed to delete item")
        }
        noteHelper.close()
    }
        ...
        return when (item.itemId) {
            R.id.action_delete -> {
                ...
                for (note in selectedNotes) {
                    deleteNote(note, adapter.listNotes.indexOf(note))
                }
                true
            }
...
2. Menerima nilai balik dari setiap aksi dan proses yang dilakukan pada NoteAddUpdateActivity
 */